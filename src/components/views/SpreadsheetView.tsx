...
const SpreadsheetView = () => {
  ...
  const [combinedPreferences, setCombinedPreferences] = useState<PreferencesMatrix>([]);
  const [selectedPairing, setSelectedPairing] = useState<Pairing | null>(null);

  // useEffect hook to load data on component mount
  useEffect(() => {
    const dfData = readSheet('Book1.xlsx', 'Sheet1');
      ...
      <table className="table-auto border-collapse border border-gray-400">
        <thead>
          <tr>
            ...
            {Array.from({ length: combinedPreferences[0]?.length || 0 }, (_, i) => (
              <th key={i} className="border border-gray-300 px-4 py-2">{`Project ${i + 1}`}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          ...
          {combinedPreferences.map((row, teamIndex) => (
            <tr key={teamIndex}>
              <td className="border border-gray-300 px-4 py-2">{`Team ${teamIndex + 1}`}</td>
